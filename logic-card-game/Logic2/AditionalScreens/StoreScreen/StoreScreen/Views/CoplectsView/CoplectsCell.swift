...
class CoplectsCell: UICollectionViewCell, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

    private var collectionView: UICollectionView!
    private var stackView: UIStackView!
    
    var viewModel: CoplectsCellModel? {
        didSet {
    ...
    private func setupCollectionView() {
        ...
        // Регистрация ячейки
        collectionView.register(CoplectsOneCell.self, forCellWithReuseIdentifier: "CoplectsOneCell")
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return viewModel?.coplectsOneCellModel.count ?? 0
    }

    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "CoplectsOneCell", for: indexPath) as? CoplectsOneCell,
            let data = viewModel?.coplectsOneCellModel[indexPath.item]
        else {
            return UICollectionViewCell()
        }
        
        let cost = PurchasedLogicHelper.shared.getCoplectionPurchased().contains(data.id) || data.cost == 0 ? "StoreScreen.Buy.Coins.Avaliable".localized() : "\(data.cost)"
        cell.configure(cost: cost, image: data.image)
        cell.updateDidSelect(isSelect: PurchasedLogicHelper.shared.getCoplectionBackId() == data.id)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if PurchasedLogicHelper.shared.getCoplectionPurchased().contains(data.id) || data.cost == 0 {
            PurchasedLogicHelper.shared.saveCoplectionBackId(data.id)
    ...
    private func buyTapped(data: CoplectsOneCellModel) {
        ...
        alert.addAction(UIAlertAction(
            ...
            handler: { [weak self] _ in
                ...
                if CoinsHelper.shared.getSpecialCoins() >= data.cost {
                    PurchasedLogicHelper.shared.addCoplectionPurchased(purchas: data.id)
